...
function lib:HandleString(str)
    local st = {}
    for utfChar in gmatch(str, "[%z\1-\127\194-\244][\128-\191]*") do
        insert(st, utfChar)
    end
    return st
end
...
function lib:GetLength()
    ...
    for i = 1, #self.string do
        L = L + ((len(self.string[i]) > 1) and 2 or 1)
    end
    return L
end
---获取字符长度
...
function lib:Sub(index, toindex)
    ...
    local i = 1
    while i < index do
        index = index - ((len(self.string[i]) > 1) and 1 or 0)
        i = i + 1
    end--处理为真正的index
    ...
    while _n <= toindex do
        if self.string[n] then
            s = s .. self.string[n]
            _n = _n + ((len(self.string[n]) > 1) and 2 or 1)
        else
            break
        end
        n = n + 1
