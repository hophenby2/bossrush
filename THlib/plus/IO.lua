...
function BinaryWriter:WriteFloat(f)
    ...
    elseif (mant == 0.0 and expo == 0) or expo < -0x7E then
        ...
        str:WriteByte(mant % 0x100)
        str:WriteByte(math.floor(mant / 0x100) % 0x100)
        str:WriteByte((expo % 0x2) * 0x80 + math.floor(mant / 0x10000))
        str:WriteByte(sign + math.floor(expo / 0x2))
    end
end
