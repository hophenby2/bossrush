...
local function FormatLastError(code)
    ...
    return string.format("%s (LastError=%d)", string.sub(__UTF16LEToUTF8(buffer), 1, -3), code)
end
...
local function UTF8ToUTF16LE(src, bytes)
    bytes = bytes or #src
    if bytes == 0 then
        return nil
    end
...
local function UTF16LEToUTF8(src, bytes)
...
local function UTF8ToANSI(src, bytes)
    --by ETC ，用于使用windows坑爹ANSI编码的函数
    --获取源字符串长度
    bytes = bytes or #src
    if bytes == 0 then
        return nil
...
function plus.EnumFiles(path)
    ...
    while true do
        ...
        if not (filename == "." or filename == "..") then
            if flag ~= 0 then
                table.insert(ret, { isDirectory = true, name = filename,real_name=filename })
            else
                table.insert(ret, { isDirectory = false, name = path.."\\"..filename, real_name=filename,size = size, lastAccessTime = lastAccessTime })
            end
        end
