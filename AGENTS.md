...
`--threat` 会放一个自动躲避机器人进去真躲，并逐帧做两件事：
**① 用「原地不动第几帧会中弹」算难度；② 量出卡片强制位移了自机多少。**

| 指标 | 含义 | 健康区间 |
|---|---|---|
| **★必须移动（次/秒）** | 原地不动会在 30 帧内中弹的次数 —— **这才是难度** | 0.5~2.5 |
...
但压力完全不同 —— 前者要求你**立刻换位置**，后者不用。所以难度 = 「你多频繁地被迫换位」，
不是「你周围有几发」。
...
9. 用 `luajit tools/check_stage.lua` 跑过吗？再跑一次 `--threat`：
   **「必须移动」在 0.5~2.5 次/秒吗？「强制位移」峰值没超过 10 px/帧吗？**
   难度曲线对吗（第一张别最难，终符最高）？
