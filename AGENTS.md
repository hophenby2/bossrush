...
`--threat` 输出的四项，是判断「这张卡到底有多难」的主要依据 ——
**别只看全屏弹数**，350 发全在屏幕另一头不等于难：

| 指标 | 含义 | 健康区间 |
|---|---|---|
| **60px 内弹数（峰值）** | 真正贴着你、要同时处理的弹 | 10~40 |
| **被堵方向（峰值 /24 格）** | 24 个 15° 扇区里有几个方向有弹 | 8~16 |
| **最大空隙（最少几格）** | 逃生窗口最窄还剩多少 | ≥ 2 格（30°） |
| **自动机被贴身（次/秒）** | 8px 内被逼近的频次（机器人很笨，只看相对高低） | 0.2~1.5 |
...
9. 用 `luajit tools/check_stage.lua` 跑过吗？再跑一次 `--threat`：
   **60px 内峰值在 10~40 吗？最大空隙还留着 ≥30° 吗？**（只看全屏弹数会被骗）
