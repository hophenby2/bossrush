...
function SUMMARY:init()
    ...
    self.bound = false
    local var = lstg.var
    self.getcardnum = boss.card.GetCardNums() - var.off_getsc
    self.maxmisscount = stage.PassMissCount[#stage.PassMissCount]
    do
        local c = scoredata.AllFinishCount
        if not ext.replay.IsReplay() and not lstg.tmpvar.InFinalStage then
            c[var.seasonid] = c[var.seasonid] + 1
        end
        ext.achievement:get(101)
        if c[var.seasonid] == 1 then
            ext.achievement:get(95 + var.seasonid)
            if c[1] > 0 and c[2] > 0 and c[3] > 0 and c[4] > 0 then
                ext.achievement:get(100)
            end
        end
        if var.dead == 0 then
            ext.achievement:get(21)
        end
        if var.getsc == self.getcardnum then
            ext.achievement:get(22)
        end
        if var.dead == 0 and var.getsc == self.getcardnum then
            ext.achievement:get(23)
        end
        if var.dead == self.maxmisscount then
            ext.achievement:get(72)
        end
        if var.getsc == 0 then
            ext.achievement:get(116)
        end
        if var.dead == self.maxmisscount and var.getsc == 0 then
            ext.achievement:get(117)
        end
        if var.score <= 500000000 then
            ext.achievement:get(121)
        end
        if var.score >= 10000000000 then
            ext.achievement:get(122)
        end
        if var.score >= 30000000000 then
            ext.achievement:get(123)
        end
        -- if var.no_man then
        --   ext.achievement:get(37)
        --end--这成就就尼玛离谱
        if #var.current_system == 0 then
    ...
    task.New(self, function()
        ...
        task.New(self, function()
            task.New(self, function()
                task.New(self, function()
                    task.SmoothSetValueTo("alpha", 0.7, 24.5 * 60, 2)
                end)
                task.SmoothSetValueTo("x", 480, 12 * 60, 2)
                task.SmoothSetValueTo("y", 270, 12 * 60, 3)
            end)
            task.Wait(14 * 60)
            task.SmoothSetValueTo("scale", 0.38, 10.5 * 60, 1)
            self.alpha = 1
            NewPulseScreen(5, nil, "mul+add", 0, 0, 60, "ui")
            task.New(self, function()
                self.white = 1
                task.SmoothSetValueTo("white", 0, 60, 1)
            end)
            local s, t = 0, 0
            while true do
        ...
        PlayMusic("SUMMARY", 0)
        for i = 1, 120 do
            SetBGMVolume("SUMMARY", i / 120)
            task.Wait()
        end
    end)
    task.New(self, function()
        task.Wait(30)
        New(units.img_obj, "title", 240, 270, nil, 100, 600, 60, 0.8, "mul+add", 150)
        task.Wait(60)
        New(units.text_obj, "作者：吃药图书", 720, 300, { 250, 128, 114 }, 90, 400, 60, 0.8, "centerpoint", 200)
        task.Wait(60)
        New(units.text_obj, "--二次创作", 880, 300, { 200, 200, 200 }, 90, 400, 60, 0.4, "centerpoint", 200)
        task.Wait(60)
        New(units.text_obj, "原作：上海爱丽丝幻乐团", 720, 240, { 230, 255, 230 }, 90, 400, 60, 0.7, "centerpoint", 200)
        task.Wait(590)
        New(units.text_obj, "Music：Eris - Yume, Maboroshi", 5, 20, { 230, 230, 230 }, 90, 200, 60, 0.4, "left", 200)
        --New(units.tag_obj, "统计与评价", 250, 380, 150, 200, 240, 240, 0.5, 20, 50)
        task.Wait(170)

        local maxgetsc = self.getcardnum
        local maxdead = self.maxmisscount
        ...
        task.Wait(55)
        self.curdead_obj = New(units.tag_obj, ("死亡数：%d / %d"):format(curdead, maxdead),
                240, 260, 150, 255, 99, 71, 0.3, 30, 35)
        task.Wait(60)
        self.cursystem_obj = New(units.tag_obj, ("携带系统数：%d / %d"):format(cursystem, maxsystem),
                260, 200, 150, 255, 227, 132, 0.1, 30, 35)
        task.Wait(60)
        New(units.tag_obj, ("读档次数：%d"):format(lstg.var.saving), 230, 140, 150, 180, 180, 180, 0.5, 30, 35)
        task.Wait(55)
        local time = int(lstg.var.elapsed)
        local k = os.date("*t", time)
        local hour, minute, second = k.hour - 8, k.min, k.sec
        local str = ""
        if hour > 0 then
            str = ("所用时间：%d : %02d : %02d"):format(hour, minute, second)
        else
            str = ("所用时间：%02d : %02d"):format(minute, second)
        end
        New(units.tag_obj, str, 230, 80, 150, 180, 180, 180, 0.5, 30, 35)
        ...
        do
            ...
            if curdead == 0 then
                ...
                proportion.dead = (1 - curdead / maxdead) * 200
            end
            proportion.score = curscore / 30000000
            proportion.getsc = proportion.getsc / 5
            proportion.dead = proportion.dead / 5
            proportion.score = proportion.score * (curgetsc / maxgetsc) * (1 - curdead / maxdead)
            ...
            self.final_score = Forbid(self.final_score, 0, 100)--最后不得超过100分
            if self.final_score <= 10 then
                ext.achievement:get(118)
            end
            if self.final_score >= 30 then
                ext.achievement:get(68)
            end
            if self.final_score >= 60 then
                ext.achievement:get(69)
            end--及格成就
            if self.final_score == 100 then
                ext.achievement:get(37)
            end--满分成就
        end--计算得分
        task.Wait(190)
...
function SUMMARY:render()
    ...
    SetImageState("summary_back", "", self.alpha * 255,
            200 + 30 * sin(self.timer), 200 + 30 * sin(self.timer), 200 + 30 * sin(self.timer))
    Render("summary_back", self.x, self.y, 0, self.scale)
    if self.z > 0 then
...
function title:render()
    ...
    ui:RenderText("big_text", self.text, self.tx, self.ty, 0.8, Color(255, 255, 255, 255), "centerpoint")
end
...
function total_score:render()
    ...
    local ang = self.angle / 90
    local angle
    for i = 1, 90 do
        angle = ang * i
        SetImageState("white", "", 150, sp:HSVtoRGB(i * 4 + self.timer / 2, self.score / self.maxscore * 0.5, 1))
        Render4V('white',
                self.x + r * cos(angle - ang), self.y + r * sin(angle - ang), 0.5,
                self.x, self.y, 0.5, self.x, self.y, 0.5,
                self.x + r * cos(angle), self.y + r * sin(angle), 0.5)
    end
    SetImageState("white", "", 255, 255, 255, 255)
    misc.SectorRender(self.x, self.y, r - 1, r + 1, 0, self.angle, 50)
